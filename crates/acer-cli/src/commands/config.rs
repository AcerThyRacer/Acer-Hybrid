...
fn set_config_value(config: &mut AcerConfig, key: &str, value: &str) -> Result<()> {
    ...
    match parts.as_slice() {
        ...
        ["providers", "ollama", "enabled"] => {
            config.providers.ollama.enabled = value.parse()
        ...
        ["providers", "openai", "enabled"] => {
            config.providers.openai.enabled = value.parse()
                .map_err(|e| anyhow::anyhow!("Invalid boolean: {}", e))?;
        }
        ...
        ["gateway", "port"] => {
            config.gateway.port = value.parse()
                .map_err(|e| anyhow::anyhow!("Invalid port: {}", e))?;
        }
        ["policy", "default", "max_cost_usd"] => {
            config.policy.default.max_cost_usd = value.parse()
                .map_err(|e| anyhow::anyhow!("Invalid number: {}", e))?;
        }
        ["policy", "default", "allow_remote"] => {
            config.policy.default.allow_remote = value.parse()
                .map_err(|e| anyhow::anyhow!("Invalid boolean: {}", e))?;
        }
        ["policy", "default", "redact_pii"] => {
            config.policy.default.redact_pii = value.parse()
                .map_err(|e| anyhow::anyhow!("Invalid boolean: {}", e))?;
        }
        _ => {
...
fn get_config_value(config: &AcerConfig, key: &str) -> Option<String> {
    ...
    match parts.as_slice() {
        ...
        ["policy", "default", "max_cost_usd"] => Some(config.policy.default.max_cost_usd.to_string()),
        ["policy", "default", "allow_remote"] => Some(config.policy.default.allow_remote.to_string()),
